...
import com.sencha.gxt.chart.client.chart.event.AxisItemUpEvent.HasAxisItemUpHandlers;
...
public abstract class Axis<M, V> implements HasAxisHandlers, HasAxisItemOutHandlers, HasAxisItemOverHandlers,
    HasAxisSelectionHandlers, HasAxisItemUpHandlers {
  ...
  public HandlerRegistration addAxisHandler(AxisHandler handler) {
    ...
    reg.add(ensureHandlers().addHandler(AxisItemUpEvent.getType(), handler));
    return reg;
  }
  ...
  @Override
  public HandlerRegistration addAxisItemUpHandler(AxisItemUpHandler handler) {
    return ensureHandlers().addHandler(AxisItemUpEvent.getType(), handler);
  }
    ...
    if (toolTip != null || handlerManager != null) {
      ...
      if (index > -1) {
        ...
        if (toolTip != null) {
          ...
          if (custom != null) {
            toolTipConfig.setBody(custom.getLabel(chart.getCurrentStore().get(index)));
          } else {
  ...
  public void onMouseUp(PrecisePoint point, Event event) {
    if (handlerManager != null) {
      int index = getIndex(point);
      if (index == labels.size()) {
        ensureHandlers().fireEvent(new AxisItemUpEvent(titleSprite.getText(), index, event));
      } else if (index > -1) {
        ensureHandlers().fireEvent(new AxisItemUpEvent(labels.get(index).getText(), index, event));
      }
    }
  }
