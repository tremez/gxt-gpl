...
public abstract class Series<M> implements HasSeriesHandlers<M>, HasSeriesSelectionHandlers<M>,
    HasSeriesItemOutHandlers<M>, HasSeriesItemOverHandlers<M>, HasSeriesItemUpHandlers<M> {
  ...
  protected int lastDisplayedTooltipIndex = -1;
  ...
  private HandlerRegistration tooltipBeforeShowHandlerRegistration;
  ...
  public HandlerRegistration addSeriesHandler(SeriesHandler<M> handler) {
    ...
    reg.add(ensureHandlers().addHandler(SeriesItemUpEvent.getType(), handler));
    return reg;
  }
  ...
  @Override
  public HandlerRegistration addSeriesItemUpHandler(SeriesItemUpHandler<M> handler) {
    return ensureHandlers().addHandler(SeriesItemUpEvent.getType(), handler);
  }
  ...
  public List<String> getLegendTitles() {
    return Collections.unmodifiableList(legendTitles);
  }
   ...
   * @param point the point clicked
   */
  public void onMouseUp(PrecisePoint point, Event event) {
    if (handlerManager != null) {
      int index = getIndex(point);
      if (index > -1) {
        ensureHandlers().fireEvent(
            new SeriesItemUpEvent<M>(chart.getCurrentStore().get(getStoreIndex(index)), getValueProvider(index), index,
                event));
      }
    }
  }
  ...
  public void removeToolTip() {
    if (tooltipBeforeShowHandlerRegistration != null) {
      tooltipBeforeShowHandlerRegistration.removeHandler();
    }
  ...
  public void setToolTipConfig(SeriesToolTipConfig<M> config) {
    ...
    if (config != null) {
      if (toolTip == null) {
        ...
        tooltipBeforeShowHandlerRegistration = toolTip.addBeforeShowHandler(new BeforeShowHandler() {
          @Override
          public void onBeforeShow(BeforeShowEvent event) {
            hideAllOtherSeriesTooltips(Series.this);
          }
        });
      } else {
  ...
  /**
   * Display the tooltip for a given point where a marker exists.
   *
   * @return if the tooltip was displayed
   */
  public boolean showTooltip(PrecisePoint point) {
    if (toolTip == null) {
      return false;
    }
    ...
    if (labelProvider != null) {
      toolTipConfig.setBody(labelProvider.getLabel(chart.getCurrentStore().get(getStoreIndex(index)),
          getValueProvider(index)));
      toolTip.update(toolTipConfig);
    }

    // Do not move the tooltip if moving over the same index
    // Display tooltip if not visible
    // Continuously move tooltip when tracking mouse
    if ((index != lastDisplayedTooltipIndex) || !toolTip.isVisible() || toolTipConfig.isTrackMouse()) {
      toolTip.showAt(
      ...
      lastDisplayedTooltipIndex = index;
    } else if (index == lastDisplayedTooltipIndex) {
      // Extend the tooltip delayHide on mousing back over the same index
      toolTip.delayHide();
    }
  ...
  /**
   * Hide all the tooltips before showing one. This ensures one visible tooltip.
   */
  protected void hideAllOtherSeriesTooltips(Series<M> showingToolTipSeries) {
    for (Series<M> series : chart.getSeries()) {
      if (series.getToolTip() != null && series.getToolTip().isVisible() && !showingToolTipSeries.equals(series)) {
