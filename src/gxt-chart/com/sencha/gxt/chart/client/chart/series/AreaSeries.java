...
public class AreaSeries<M> extends MultipleColorSeries<M> {
  ...
  public void highlightAll(int index) {
    if (sprites.size() > index) {
      highlighter.highlight(sprites.get(index));
    }
  }
  ...
  public void unHighlightAll(int index) {
    if (sprites.size() > index) {
      highlighter.unHighlight(sprites.get(index));
    }
  }
  ...
  private void calculateBounds() {
    ...
    if (Double.isNaN(min.getX())) {
      min.setX(0);
      if (chart.getCurrentStore().size() == 1) {
        // work with only one data point
        xScale = bbox.getWidth();
      }  else {
      ...
      if (labelPosition == LabelPosition.OUTSIDE) {
        ...
        for (int i = 0; i < yValues.size(); i++) {
          ...
          for (int j = 0; j < values.length; j++) {
            ...
            SeriesRenderer<M> spriteRenderer = labelConfig.getSpriteRenderer();
            if(spriteRenderer != null) {
              spriteRenderer.spriteRenderer(sprite, i * values.length + j, chart.getCurrentStore());
            }
            double offsetY = sprite.getBBox().getHeight() / 2.0;
