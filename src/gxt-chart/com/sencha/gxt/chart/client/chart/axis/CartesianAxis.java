...
public abstract class CartesianAxis<M, V> extends Axis<M, V> {
  ...
  public void drawAxis(boolean init) {
    ...
    if (position == Position.LEFT || position == Position.RIGHT) {
      ...
      while (currentX <= x + gutterX + trueLength) {
        commands.add(new MoveTo(Math.floor(currentX) + 0.5, currentY));
        commands.add(new LineTo(0, dashSize * 2 + 1, true));
        if (currentX != x + gutterX) {
          for (int i = 1; i < dashesX; i++) {
            commands.add(new MoveTo(Math.floor(currentX - delta * i / dashesX) + 0.5, currentY));
      ...
      if (Math.round(currentX - delta - (x + gutterX + trueLength)) != 0) {
        commands.add(new MoveTo(Math.floor(x + length - gutterX) + 0.5, currentY));
        commands.add(new LineTo(0, dashSize * 2 + 1, true));
        for (int i = 1; i < dashesX; i++) {
          commands.add(new MoveTo(Math.floor(x + length - gutterX - delta * i / dashesX) + 0.5, currentY));
          commands.add(new LineTo(0, dashSize + 1, true));
        }
        ticks.add(new PrecisePoint(Math.floor(currentX), Math.floor(y)));
      }
  ...
  public void drawLabels() {
    ...
    if (titleConfig == null) {
      ...
      double x = this.x;
      double y = this.y;
      boolean rotate = position.isVertical();
      if (titleSprite == null) {
      ...
      if (rotate) {
        y -= this.length / 2.0 - titleBBox.getHeight() / 2.0;
        if (position == Position.LEFT) {
          x -= (maxWidth + textLabelPadding + titleBBox.getWidth());
        } else {
          x += (maxWidth + textLabelPadding - (titleBBox.getWidth() / 2.0));
        }
        bbox.setWidth(maxWidth + titleBBox.getWidth());
        bbox.setHeight(chart.getBBox().getHeight());
      } else {
        x += (this.length / 2.0) - (titleBBox.getWidth() / 2.0);
        if (position == Position.TOP) {
          y -= (maxHeight + pad + titleBBox.getHeight());
        } else {
          y += (maxHeight + pad - (titleBBox.getHeight() / 2.0));
        }
        bbox.setHeight(maxHeight + titleBBox.getHeight());
        bbox.setWidth(chart.getBBox().getWidth());
      }
  ...
  private void drawGrid() {
    ...
    if ((gutters[1] != 0 && position.isVertical())
        || (gutters[0] != 0 && !position.isVertical())) {
      i = 0;
    }
    ...
    // even path
    if (evenCommands.size() > 0 && gridEvenConfig != null) {
      if (gridEven == null) {
    ...
    // standard grid
    if (commands.size() > 0 && defaultGridConfig != null && gridEvenConfig == null && gridOddConfig == null) {
      if (gridLines == null) {
