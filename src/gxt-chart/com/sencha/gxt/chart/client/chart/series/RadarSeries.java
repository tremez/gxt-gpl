...
public class RadarSeries<M> extends ScatterSeries<M> {
  ...
  public void drawSeries() {
    ...
    coordinates = new PrecisePoint[len];
    ...
    for (int i = 0; i < len; i++) {
      ...
      x = rho * Math.cos(i / ((double) len) * Math.PI * 2);
      y = rho * Math.sin(i / ((double) len) * Math.PI * 2);
      if (i == 0) {
        commands.add(new MoveTo(x + centerX, y + centerY));
      } else {
        commands.add(new LineTo(x + centerX, y + centerY));
      }
      coordinates[i] = new PrecisePoint(x + centerX, y + centerY);
    }
