...
public class PathSprite extends Sprite {
  ...
  private LineCap strokeLineCap = LineCap.BUTT;
  private boolean strokeLineCapDirty = true;
  private LineJoin strokeLineJoin = LineJoin.MITER;
  private boolean strokeLineJoinDirty = true;
  private double miterLimit = 4;
  private boolean miterLimitDirty = true;
  ...
  /**
   * Returns a {@link List} of all the {@link PathCommand}s in the sprite. Note that modifying this will not cause the
   * path to be marked as dirty, either use the other command manipulation methods, or force redraw some other way.
   *
   * @see #addCommand(int, PathCommand)
   * @see #addCommand(PathCommand)
   * @see #clearCommands()
   * @see #setCommand(int, PathCommand)
   * @see #setCommands(java.util.List)
  ...
  public void setCommand(int index, PathCommand command) {
    ...
    if (isCurved() && !(command instanceof CurveTo)) {
      this.setCurved(false);
    }
    if (isAbsolute() && command.isRelative()) {
      this.setAbsolute(false);
    }
  ...
  protected PreciseRectangle curveDimensions(PrecisePoint start, CurveTo curve) {
    ...
    double b = 2.0 * (curve.getX1() - start.getX()) - 2.0 * (curve.getX2() - curve.getX1());
    double c = start.getX() - curve.getX1();
    double t1 = (-b + Math.sqrt(b * b - 4.0 * a * c)) / 2.0 / a;
    double t2 = (-b - Math.sqrt(b * b - 4.0 * a * c)) / 2.0 / a;
    PrecisePoint min = new PrecisePoint(Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY);
    PrecisePoint max = new PrecisePoint(Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY);
    min.setX(Math.min(start.getX(), curve.getX()));
    min.setY(Math.min(start.getY(), curve.getY()));
    max.setX(Math.max(start.getX(), curve.getX()));
    max.setY(Math.max(start.getY(), curve.getY()));
    ...
    if (t1 > 0 && t1 < 1) {
      ...
      min.setX(Math.min(min.getX(), dot.getX()));
      min.setY(Math.min(min.getY(), dot.getY()));
      max.setX(Math.max(max.getX(), dot.getX()));
      max.setY(Math.max(max.getY(), dot.getY()));
    }
    if (t2 > 0 && t2 < 1) {
